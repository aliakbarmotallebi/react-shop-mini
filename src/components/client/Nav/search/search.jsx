...
export default function Search() {
    ...
    const searchFormSubmit = (event) => {
        ...
        router.push(`/shop?q=${searchedQuery}`)
        removeSearchedBox()
    }

    useEffect(() => {
        const { search, search_count } = Config.services.search
        if (searchedQuery.length <= 1) return
        setIsLoading(true)
    ...
    return (
        <>
            <form onSubmit={searchFormSubmit} className='relative' id='searchForm'>
                ...
                {isLoading ? <SearchLoadin /> : <SearchSuggest searchedItems={searchedItems} searchedQuery={searchedQuery} removeSearchedBox={removeSearchedBox} />}
            </form>
        </>
